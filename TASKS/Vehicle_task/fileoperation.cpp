...
void FileOperation::writeBikeData(std::list<Rentalbike>bikeList)
{
    ...
    if(!file)
    {
        cout<<"Not able to open the file for writing"<<endl;
        return;
    }
    for(auto i:bikeList)
    {
        file<<i.getBrand()<<","<<i.getModel()<<","<<i.getVehicleNumber()<<","<<i.getRentPrice()<<","<<i.getStatus()<<endl;
    }
    file.close();
    cout<<"Bike data written to csv file successfully"<<endl;
}

std::list<Rentalbike> FileOperation::readBikeData()
{
    std::list<Rentalbike> bikeList;
    std::ifstream file("BikeData.csv");
    if(!file)
    {
    ...
    while(std::getline(file,brand,',') && std::getline(file,model,',')&& std::getline(file,vehicleNumber,',')
           && file>>rentPrice && file.ignore() && std::getline(file,status))
    {
...
void FileOperation::writeCarData(std::list<Rentalcar> carList)
{
    ...
    csvCarFile.close();
    cout<<"Data written to car file successfully"<<endl;
}
...
void FileOperation::writecustomerdata(std::list<Customer> Customerlist)
{
    ...
    cout<<"bike customer data written to file successfully"<<endl;
...
void FileOperation::writecustomerdatacar(std::list<Customer> Customerlist)
{
    ...
    csvCustomerFile.close();
    cout<<"car customer data written to file successfully"<<endl;
}
